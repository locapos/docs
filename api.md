...
```javascript
[{"id":"opaque-id","name":"screen-name","latitude":35,"longitude":135,"heading":40}]
```
...
```javascript
{"provider":"provider-name","id":"opaque-id","name":"screen-name"}
```

`provider`にはログインに使用したIdPに応じて次の値が設定されます。
